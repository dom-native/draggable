...
export function activateDrag(src: HTMLElement, evt: PointerEvent, controller?: DragController) {
	...
	const ghost = (dragTarget === 'ghost') ? createGhost(source, ctlr) : undefined;
...
function createGhost(source: HTMLElement, controller: DragControllerInternal) {
	const createFn = controller.ghost?.create;

	const ghost = (createFn) ? createFn(source) : source.cloneNode(true) as HTMLElement;
...
//#region    ---------- DOM Utils ---------- 
function findBelow(x: number, y: number, el?: HTMLElement) {
